...
#[derive(Debug, Clone)]
#[allow(missing_docs)]
pub struct PaddingCombinationsConfig<F, K> {
    pub byte_range_col: TableColumn,
    pub s_flag_col: TableColumn,
    _marker: PhantomData<F>,
}

impl<F: Field, K: u32> PaddingCombinationsConfig<F, K> {
    // #[allow(dead_code)]
...
pub struct KeccakPaddingConfig<F> {
    ...
    _marker: PhantomData<F>,
    byte_table: PaddingCombinationsConfig<F, 255>,
}
...
impl<F: Field> KeccakPaddingConfig<F> {
    pub(crate) fn configure(meta: &mut ConstraintSystem<F>) -> Self {
        ...
        let byte_table = PaddingCombinationsConfig::<F, 255>::configure(meta);
